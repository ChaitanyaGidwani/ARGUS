...
export function OpportunityCard({ opp, saved, onSave, onOpen, variant = "compact" }: Props) {
  ...
  if (variant === "featured") {
    return (
      ...
      >
        <div className="absolute inset-0 opacity-90 dark:opacity-30 pointer-events-none" style={{ background: `radial-gradient(circle at top right, ${cat.color}26, transparent 65%), radial-gradient(circle at bottom left, ${cat.color}14, transparent 60%)` }} />
        <div className="relative flex flex-col gap-3 h-full">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium px-2.5 py-1 rounded-lg" style={{ background: cat.bg, color: cat.color }}>
              <cat.icon className="inline w-3 h-3 mr-1" />{opp.category}
            </span>
            <span
              role="button"
              tabIndex={0}
          ...
          <div className="mt-auto flex items-center justify-between pt-2">
            ...
            <span className="text-sm font-bold" style={{ color: cat.color }}>{opp.prize}</span>
          </div>
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between mb-2">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: cat.bg }}>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <span className="text-[10px] font-medium px-2 py-0.5 rounded" style={{ background: cat.bg, color: cat.color }}>{opp.type}</span>
        <Bookmark
          onClick={(e) => { e.stopPropagation(); onSave(opp.id); }}
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-bold" style={{ color: cat.color }}>{opp.prize}</span>
      </div>
    </button>
  );
}
